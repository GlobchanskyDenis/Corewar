...
/*   Updated: 2020/02/15 18:54:09 by volyvar-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*step = -1;
	fprint("Error: or function error at %d position\n", position);
	return (-1);
}
...
void				operation_or(t_dasm *dasm, int position, int *step)
{
	...
	arg1.size = get_arg_size(types >> 6, 7);
	arg2.size = get_arg_size((types >> 4) & 3, 7);
	arg3.size = get_arg_size((types >> 2) & 3, 7);
