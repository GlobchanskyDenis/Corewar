...
/*   Updated: 2020/02/15 18:53:47 by volyvar-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*step = -1;
	fprint("Error: ldi function error at %d position\n", position);
	return (-1);
}
...
void				operation_ldi(t_dasm *dasm, int position, int *step)
{
	...
	arg1.size = get_arg_size(types >> 6, 10);
	arg2.size = get_arg_size((types >> 4) & 3, 10);
	arg3.size = get_arg_size((types >> 2) & 3, 10);
	...
	arg3.val = get_bytes(dasm->code, position + 2 + arg1.size + arg2.size, \
			arg3.size, dasm);
	if (is_invalid_parameters(types) && \
			error(position, step))
		return ;
	fprint_fd(dasm->fd_o, "\tldi ", arg1);
