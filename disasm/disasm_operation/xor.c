...
/*   Updated: 2020/02/15 18:54:32 by volyvar-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*step = -1;
	fprint("Error: xor function error at %d position\n", position);
	return (-1);
}
...
void				operation_xor(t_dasm *dasm, int position, int *step)
{
	...
	arg1.size = get_arg_size(types >> 6, 8);
	arg2.size = get_arg_size((types >> 4) & 3, 8);
	arg3.size = get_arg_size((types >> 2) & 3, 8);
